                ...
                with dpg.child_window(width=250, no_scrollbar=True):
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_input_float(
                            ...
                            callback=lambda s, v, u: control_points_manager.update_control_point_field(u, "position", v),
                            user_data=point["id"]
                        )
                        dpg.add_combo(
                            ...
                            width=60,
                            callback=lambda s, v, u: control_points_manager.update_control_point_field(u, "unit_position", v),
                            user_data=point["id"]
                        )
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_input_float(
                            ...
                            width=-65, format="%.2f", min_value=0.0, min_clamped=True,
                            callback=lambda s, v, u: control_points_manager.update_control_point_field(u, "cw", v),
                            user_data=point["id"]
                        )
                        dpg.add_combo(
                            ...
                            width=60,
                            callback=lambda s, v, u: control_points_manager.update_control_point_field(u, "unit_cw", v),
                            user_data=point["id"]
                        )
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_input_float(
                            ...
                            width=-65, format="%.2f", min_value=0.0, min_clamped=True,
                            callback=lambda s, v, u: control_points_manager.update_control_point_field(u, "ch", v),
                            user_data=point["id"]
                        )
                        dpg.add_combo(
                            ...
                            width=60,
                            callback=lambda s, v, u: control_points_manager.update_control_point_field(u, "unit_ch", v),
                            user_data=point["id"]
                        )
                    ...
                    if point["id"] != "outlet":
                        dpg.add_button(
                            ...
                            width=-1,
                            callback=lambda s, a, u: (control_points_manager.add_control_point(u), build_cards(control_points_manager)),
                            user_data=point["id"]
                        )
                    ...
                    if not point["fixed"]:
                        dpg.add_button(
                            label="Delete",
                            width=-1,
                            callback=lambda s, a, u: (control_points_manager.delete_control_point(u), build_cards(control_points_manager)),
                            user_data=point["id"]
                        )
    ...
    with dpg.window(tag="main_window", no_title_bar=True, no_resize=True, no_move=True, no_scrollbar=False, width=1200, height=760):
        ...
        with dpg.tab_bar(tag="main_tab_bar"):
                ...
                with dpg.table(header_row=False, borders_innerV=True, policy=dpg.mvTable_SizingStretchProp):
                    ...
                    with dpg.table_row():
                        ...
                        with dpg.table_cell():
                            ...
                            with dpg.table(header_row=False, policy=dpg.mvTable_SizingStretchProp):
                                ...
                                with dpg.table_row():
                                    with dpg.table_cell():
                                        ...
                                        dpg.add_combo(tag="input_coolant", items=COOLANT_ITEMS, width=-1)
                                    with dpg.table_cell():
                                        dpg.add_spacer(height=6)
                            ...
                            with dpg.table(header_row=False, policy=dpg.mvTable_SizingStretchProp):
                                ...
                                with dpg.table_row():
                                    with dpg.table_cell():
                                        ...
                                        dpg.add_combo(tag="interpolation_type", items=INTERPOLATION_TYPE_ITEMS, width=-1)
                                    with dpg.table_cell():
                                        dpg.add_spacer(height=5)
                            ...
                            build_cards(dynamic.control_points_manager)
            ...
            with dpg.tab(label="Solver & Results"):
                    ...
                    with dpg.table_row():
                        with dpg.table_cell():
                            with dpg.group(horizontal=True):
                                ...
                                dpg.add_combo(tag="input_pressure_drop_model", items=PRESSURE_DROP_MODEL_ITEMS, callback=callbacks.on_pressure_drop_model_change, width=-1)
                        with dpg.table_cell():
                            with dpg.group(horizontal=True):
                        ...
                        with dpg.table_cell():
                            with dpg.group(horizontal=True):
                                ...
                                dpg.add_combo(tag="input_hot_side_model", items=HOT_SIDE_MODEL_ITEMS, callback=callbacks.on_hot_side_model_change, width=-1)
                        with dpg.table_cell():
                            with dpg.group(horizontal=True):
                ...
                with dpg.table(header_row=False, policy=dpg.mvTable_SizingStretchProp):
                    dpg.add_table_column(init_width_or_weight=0.08)
                ...
                with dpg.child_window(width=-1, height=-1, border=True, no_scrollbar=True, no_scroll_with_mouse=True):
                    with dpg.child_window(tag="results_output_window", width=-1, height=-35, border=False):
                        pass

                    # Footer
                    with dpg.child_window(tag="footer", width=-1, height=35, border=False):
                        dpg.add_separator()
                        dpg.add_spacer(height=1)
                        with dpg.table(header_row=False, policy=dpg.mvTable_SizingStretchProp):
                            ...
                            with dpg.table_row():
                                ...
                                with dpg.table_cell():
                                    with dpg.group(horizontal=True):
                                        dpg.add_button(label="Save As", callback=lambda: callbacks.on_write_full_pyregen_output(state))
